...
def calcul():
    ...
    chemin = nx.dijkstra_path(G, position, destination)
    distance = nx.dijkstra_path_length(G, position, destination)

    return render_template("resultat.html", data=[position, destination, chemin, distance])
